...
public class ClienteRMI extends JFrame implements ActionListener {
	...
	public ClienteRMI() {
		...
		// -----------------------------------------
		/*
		 * interceptar el método de cierre, informar al servidor que nos vamos y luego
		...
		frame.addWindowListener(new java.awt.event.WindowAdapter() {
			...
			public void windowClosing(java.awt.event.WindowEvent windowEvent) {
				...
				if (chatClient != null) {
					try {
						sendMessage("Adiós a todos, me voy");
		...
		try {
			...
			if (e.getSource() == startButton) {
				...
				if (name.length() != 0) {
					...
					textField.setText("");
					textArea.append("username : " + name + " conectándose al chat...\n");
					getConnected(name);
